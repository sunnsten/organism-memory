...
class Qwen3Backend(LMBackend):
    """Backend for Qwen/Qwen3-* models."""
    ...
    def __init__(
        ...
        print(f"[Qwen3Backend] Loading model: {model_name} (dtype={torch_dtype}, device_map={device_map})")

        self.tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=True)
        logger.debug("Tokenizer loaded")
        print("[Qwen3Backend] Tokenizer loaded")
        ...
        if load_in_4bit or load_in_8bit:
            try:
                ...
                if load_in_4bit:
                    ...
                    print(f"[Qwen3Backend] 4-bit NF4 quantization enabled (compute dtype={torch_dtype})")
                else:
                    quantization_config = BitsAndBytesConfig(load_in_8bit=True)
                    print("[Qwen3Backend] 8-bit quantization enabled")
                # Force all layers onto GPU 0; "auto" can spill to CPU if accelerate
                # overestimates VRAM need, which silently degrades throughput.
        ...
        if use_device_map is None and quantization_config is None:
        ...
        print(f"[Qwen3Backend] Model loaded on device: {final_device}")

        # Qwen3 may not have pad_token — fall back to eos_token
